...
def convert_to_mt(row):
    ...
    if unit in ('NOS', 'CTN', 'CTNS', 'CTM', 'CARTONS', 'CARTON', 'PCS', 'PKGS'):
        ...
        # A: NET WT pattern — "NET WT:1.200 KGS" / "NET WT:0.600KGS" / "NET.WT:1.200 KGS"
        ...
        # D: "X KGS X Y" — e.g. "0.2 KGS X 12 PCS" / "INSTANT COFFEE 0.2 KGS X 12 PCS"
        ...
        # J: "X GRAMS EACH" e.g. "50 GRAMS EACH TIN", "PACKING : 50 GRAMS EACH"
        if weight_g is None:
            m = re.search(r'(\d+(?:\.\d+)?)\s*GRAMS?\s*EACH\b', desc)
            if m:
